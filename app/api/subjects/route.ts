...
import { ensureSubjectCatalogMigration, ensureSubjectCreditsMigration, ensureSubjectStudyTrackingMigration } from "@/lib/data-migrations";
...
export async function GET() {
  ...
  await ensureSubjectCreditsMigration(db, userId);
...
export async function POST(request: Request) {
  ...
  await db.collection<SubjectDocument>("subjects").updateOne(
    ...
    { $set: { name: parsed.data.name, credits: parsed.data.credits ?? existing?.credits ?? 3, normalizedName, aliases, updatedAt: now }, $setOnInsert: { userId, id: parsed.data.id, weeklyStudyTrackingStartDate: today, createdAt: now } },
    { upsert: true },
  );
