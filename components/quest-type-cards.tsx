...
export function QuestTypeCards({ label, options, selectedId, variant = "compact", onSelect, onManage }: Props) {
  return <fieldset className={`quest-type-fieldset quest-type-${variant}`}>
    <legend>{label}</legend>
    <div className="quest-type-cards" role="radiogroup" aria-label={label}>
      {options.map((option, index) => <button key={option.id} type="button" role="radio" aria-checked={selectedId === option.id} className={`quest-type-card tone-${option.tone ?? index % 5} ${selectedId === option.id ? "selected" : ""}`} onClick={() => onSelect(option.id)}>
        <i>{String(index + 1).padStart(2, "0")}</i><span><strong>{option.label}</strong>{option.detail && <small>{option.detail}</small>}</span>{selectedId === option.id && <b className="quest-type-selected" aria-hidden="true"><Check size={12} /></b>}
      </button>)}
