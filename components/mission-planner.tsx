    ...
    <main className={`app-shell ${studyTimer.session ? "timer-active" : ""}`} data-theme={theme}>
      ...
      <section className={`content ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
        <header className="topbar">
          ...
          <div className="search-box" aria-busy={query !== deferredQuery}><Search size={18} /><input aria-label="Buscar" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={view === "world" ? "Buscar tarea o materia..." : view === "map" ? "Buscar un destino..." : view === "weekly" ? "Buscar tarea o actividad..." : "Buscar una misión..."} /></div>
        ...
        {view === "world" ? (
          ...
          <div className="workspace weekly-workspace">
            <WeeklySchedule weeklyQuests={catalogWeeklyQuests} loading={scheduleLoading || activityTypesLoading} focusedWeeklyQuestId={focusedWeeklyQuestId} subjects={subjects} activityTypes={activityTypes} missions={missions} query={normalizedQuery} activeTimerMissionId={studyTimer.session?.missionId} onManageSubjects={() => setView("world")} onOpenMission={openEdit} onCreateMission={() => openNew(selectedDate)} onStartTimer={startStudyTimer} onSave={upsertWeeklyQuest} onDelete={removeWeeklyQuest} onSaveActivityType={upsertActivityType} onDeleteActivityType={removeActivityType} />
          </div>
