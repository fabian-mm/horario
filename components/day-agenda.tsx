      ...
      {events.map((event) => {
        ...
        if (event.kind === "activity") {
          ...
          return <article key={event.id} style={style} className={`day-agenda-event activity-event activity-tone-${type.tone} ${activity.completed ? "completed" : ""}`} onClick={() => onOpenActivity(activity)}>
            ...
            <div><time><Clock3 size={11} />{formatTimeRange12Hour(activity.startTime, activity.endTime)}</time><strong>{getScheduledActivityLabel(activity)}</strong><small>{activity.activityCategory === "class" ? "ACTIVIDAD · MATERIA" : "ACTIVIDAD GENERAL"}{activity.location ? <><MapPin size={10} />{activity.location}</> : null}</small></div>
          </article>;
        }
